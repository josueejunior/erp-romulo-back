...
class RedisService
{
     ...
     * Útil quando senha é alterada
     * 
     * Nota: Como a chave de login usa hash(email + senha), não podemos
     * invalidar todas as chaves de um email diretamente. Este método
     * busca todas as chaves de login e filtra por email.
     * Para melhor performance, considere usar TTL menor no cache de login.
     */
    public static function invalidateLoginCache(string $email): void
    {
        if (!self::isAvailable()) {
        ...
        try {
            // Buscar todas as chaves de login
            // A chave é: login:md5(email:passwordHash)
            // Como não sabemos o passwordHash, precisamos buscar todas
            $pattern = "login:*";
            $cursor = 0;
            $deleted = 0;
            ...
            do {
                ...
                if (!empty($keys)) {
                    foreach ($keys as $key) {
                        // A chave contém o hash de email:passwordHash
                        // Não podemos validar diretamente, então deletamos todas
                        // as chaves de login (serão recriadas no próximo login)
                        // Alternativa: usar TTL menor no cache de login
                        Redis::del($key);
                        $deleted++;
                    }
                }
            ...
            if ($deleted > 0) {
                Log::debug("Cache de login invalidado", [
                    'email' => $email,
                    'chaves_deletadas' => $deleted
                ]);
            }
        } catch (\Exception $e) {
            Log::warning('Erro ao invalidar cache de login: ' . $e->getMessage());
    ...
    public static function clearAuthCache(): void
    {
        ...
        try {
            ...
            Log::info("Cache de autenticação limpo", ['chaves_deletadas' => $totalDeleted]);
        } catch (\Exception $e) {
            Log::warning('Erro ao limpar cache de autenticação: ' . $e->getMessage());
        }
    }
