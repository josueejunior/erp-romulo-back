...
@section('title', 'Editar AF - ' . $processo->numero_modalidade)
...
<div class="mb-8 flex items-center gap-4">
    ...
    <div>
        <h1 class="text-3xl font-bold text-gray-900">Editar Autorização de Fornecimento</h1>
        <p class="text-gray-600 mt-1">Processo: {{ $processo->numero_modalidade }}</p>
    </div>
</div>

<div class="card p-8">
    <form method="POST" action="{{ route('autorizacoes-fornecimento.update', [$processo, $autorizacaoFornecimento]) }}">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                ...
                <input type="text" name="numero" required
                       ...
                       value="{{ old('numero', $autorizacaoFornecimento->numero) }}">
                @error('numero')
            ...
            <div>
                ...
                <input type="date" name="data" required
                       class="w-full border border-gray-300 rounded-lg px-4 py-3 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors" 
                       value="{{ old('data', $autorizacaoFornecimento->data->format('Y-m-d')) }}">
                @error('data')
                    <p class="mt-2 text-sm text-red-600 flex items-center gap-1">
                        <x-heroicon-s-exclamation-circle class="w-4 h-4" />
                        {{ $message }}
            ...
            <div>
                ...
                <input type="number" name="valor" required step="0.01" min="0"
                       class="w-full border border-gray-300 rounded-lg px-4 py-3 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors" 
                       value="{{ old('valor', $autorizacaoFornecimento->valor) }}" placeholder="0.00">
                @error('valor')
                    <p class="mt-2 text-sm text-red-600 flex items-center gap-1">
                        <x-heroicon-s-exclamation-circle class="w-4 h-4" />
                        {{ $message }}
            ...
            <div>
                ...
                <select name="situacao" required
                        class="w-full border border-gray-300 rounded-lg px-4 py-3 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors">
                    <option value="aguardando_empenho" {{ old('situacao', $autorizacaoFornecimento->situacao) == 'aguardando_empenho' ? 'selected' : '' }}>Aguardando Empenho</option>
                    <option value="atendendo" {{ old('situacao', $autorizacaoFornecimento->situacao) == 'atendendo' ? 'selected' : '' }}>Atendendo</option>
                    <option value="concluida" {{ old('situacao', $autorizacaoFornecimento->situacao) == 'concluida' ? 'selected' : '' }}>Concluída</option>
                </select>
            </div>
            ...
            <div class="md:col-span-2">
                ...
                <select name="contrato_id"
                    ...
                    <option value="{{ $contrato->id }}" {{ old('contrato_id', $autorizacaoFornecimento->contrato_id) == $contrato->id ? 'selected' : '' }}>
            ...
            <div>
                ...
                <input type="text" value="R$ {{ number_format($autorizacaoFornecimento->saldo, 2, ',', '.') }}" disabled
            ...
            <div class="md:col-span-2">
                ...
                <textarea name="observacoes" rows="3"
                          ...
                          placeholder="Observações adicionais...">{{ old('observacoes', $autorizacaoFornecimento->observacoes) }}</textarea>
            </div>
        </div>
