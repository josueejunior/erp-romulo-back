...
@section('title', 'Editar Item - ' . $processo->numero_modalidade)
...
<div class="mb-8 flex items-center gap-4">
    ...
    <div>
        <h1 class="text-3xl font-bold text-gray-900">Editar Item {{ $item->numero_item }}</h1>
        <p class="text-gray-600 mt-1">Processo: {{ $processo->numero_modalidade }}</p>
    </div>
</div>

<div class="card p-8">
    <form method="POST" action="{{ route('processo-itens.update', [$processo, $item]) }}">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <input type="number" name="quantidade" required step="0.01" min="0.01"
                       class="w-full border border-gray-300 rounded-lg px-4 py-3 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors" 
                       value="{{ old('quantidade', $item->quantidade) }}" placeholder="0.00">
                @error('quantidade')
                    <p class="mt-2 text-sm text-red-600 flex items-center gap-1">
                        <x-heroicon-s-exclamation-circle class="w-4 h-4" />
                        {{ $message }}
            ...
            <div>
                ...
                <input type="text" name="unidade" required maxlength="50"
                       class="w-full border border-gray-300 rounded-lg px-4 py-3 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors" 
                       value="{{ old('unidade', $item->unidade) }}" placeholder="Ex: UN, KG, M, M², etc">
                @error('unidade')
                    <p class="mt-2 text-sm text-red-600 flex items-center gap-1">
                        <x-heroicon-s-exclamation-circle class="w-4 h-4" />
                        {{ $message }}
            ...
            <div>
                ...
                <input type="number" name="valor_estimado" step="0.01" min="0"
                       class="w-full border border-gray-300 rounded-lg px-4 py-3 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors" 
                       value="{{ old('valor_estimado', $item->valor_estimado) }}" placeholder="0.00">
            ...
            <div class="md:col-span-2">
                ...
                <textarea name="especificacao_tecnica" required rows="5"
                          ...
                          placeholder="Descreva detalhadamente a especificação técnica do item...">{{ old('especificacao_tecnica', $item->especificacao_tecnica) }}</textarea>
            ...
            <div>
                ...
                <input type="text" name="marca_modelo_referencia" maxlength="255"
                       class="w-full border border-gray-300 rounded-lg px-4 py-3 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors" 
                       value="{{ old('marca_modelo_referencia', $item->marca_modelo_referencia) }}" placeholder="Ex: Marca X, Modelo Y">
            ...
            <div>
                <label class="flex items-center gap-2 mt-8">
                    <input type="checkbox" name="exige_atestado" value="1" 
                           {{ old('exige_atestado', $item->exige_atestado) ? 'checked' : '' }}
            ...
            <div id="quantidade_atestado_field" style="display: {{ old('exige_atestado', $item->exige_atestado) ? 'block' : 'none' }};">
                <label class="block text-sm font-semibold text-gray-700 mb-2">
                    Quantidade Mínima no Atestado
                </label>
                <input type="number" name="quantidade_minima_atestado" min="1"
                       class="w-full border border-gray-300 rounded-lg px-4 py-3 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors" 
                       value="{{ old('quantidade_minima_atestado', $item->quantidade_minima_atestado) }}" placeholder="Quantidade mínima">
            ...
            <div class="md:col-span-2">
                ...
                <textarea name="observacoes" rows="3"
                          class="w-full border border-gray-300 rounded-lg px-4 py-3 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors resize-none"
                          placeholder="Observações adicionais...">{{ old('observacoes', $item->observacoes) }}</textarea>
            </div>
        </div>
