...
<div class="mb-8 flex items-center gap-4">
    <a href="{{ route('orgaos.show', $setor->orgao) }}" class="text-gray-600 hover:text-gray-900">
        <x-heroicon-o-arrow-left class="w-6 h-6" />
    </a>
    <div>
        <h1 class="text-3xl font-bold text-gray-900">Editar Setor</h1>
        <p class="text-gray-600 mt-1">{{ $setor->orgao->razao_social }}</p>
    </div>
</div>

<div class="card p-8">
    <form method="POST" action="{{ route('setors.update', $setor) }}">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div class="md:col-span-2">
                ...
                <input type="text" value="{{ $setor->orgao->razao_social }}" disabled
            ...
            <div class="md:col-span-2">
                ...
                <input type="text" name="nome" required 
                       ...
                       value="{{ old('nome', $setor->nome) }}" placeholder="Nome do setor">
                @error('nome')
            ...
            <div>
                ...
                <input type="email" name="email" 
                       class="w-full border border-gray-300 rounded-lg px-4 py-3 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors" 
                       value="{{ old('email', $setor->email) }}" placeholder="email@exemplo.com">
                @error('email')
                    <p class="mt-2 text-sm text-red-600 flex items-center gap-1">
                        <x-heroicon-s-exclamation-circle class="w-4 h-4" />
                        {{ $message }}
            ...
            <div>
                ...
                <input type="text" name="telefone" 
                       class="w-full border border-gray-300 rounded-lg px-4 py-3 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors" 
                       value="{{ old('telefone', $setor->telefone) }}" placeholder="(00) 00000-0000">
            </div>
            ...
            <div class="md:col-span-2">
                ...
                <textarea name="observacoes" rows="4" 
                          ...
                          placeholder="Observações adicionais...">{{ old('observacoes', $setor->observacoes) }}</textarea>
            </div>
        </div>
        ...
        <div class="mt-8 flex gap-4 pt-6 border-t border-gray-200">
            ...
            <a href="{{ route('orgaos.show', $setor->orgao) }}" class="bg-gray-200 text-gray-700 px-8 py-3 rounded-lg hover:bg-gray-300 transition-colors font-semibold">
                Cancelar
            </a>
        </div>
    </form>
