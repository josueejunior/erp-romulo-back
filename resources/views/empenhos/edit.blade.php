...
@section('title', 'Editar Empenho - ' . $processo->numero_modalidade)
...
<div class="mb-8 flex items-center gap-4">
    ...
    <div>
        <h1 class="text-3xl font-bold text-gray-900">Editar Empenho</h1>
        <p class="text-gray-600 mt-1">Processo: {{ $processo->numero_modalidade }}</p>
    </div>
</div>

<div class="card p-8">
    <form method="POST" action="{{ route('empenhos.update', [$processo, $empenho]) }}">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                ...
                <input type="text" name="numero" required
                       ...
                       value="{{ old('numero', $empenho->numero) }}">
                @error('numero')
            ...
            <div>
                ...
                <input type="date" name="data" required
                       class="w-full border border-gray-300 rounded-lg px-4 py-3 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors" 
                       value="{{ old('data', $empenho->data->format('Y-m-d')) }}">
                @error('data')
                    <p class="mt-2 text-sm text-red-600 flex items-center gap-1">
                        <x-heroicon-s-exclamation-circle class="w-4 h-4" />
                        {{ $message }}
            ...
            <div>
                ...
                <input type="number" name="valor" required step="0.01" min="0"
                       class="w-full border border-gray-300 rounded-lg px-4 py-3 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors" 
                       value="{{ old('valor', $empenho->valor) }}" placeholder="0.00">
                @error('valor')
                    <p class="mt-2 text-sm text-red-600 flex items-center gap-1">
                        <x-heroicon-s-exclamation-circle class="w-4 h-4" />
                        {{ $message }}
            ...
            <div>
                ...
                <input type="date" name="data_entrega"
                       class="w-full border border-gray-300 rounded-lg px-4 py-3 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors" 
                       value="{{ old('data_entrega', $empenho->data_entrega ? $empenho->data_entrega->format('Y-m-d') : '') }}">
            </div>
            ...
            <div>
                ...
                <select name="contrato_id"
                    ...
                    <option value="{{ $contrato->id }}" {{ old('contrato_id', $empenho->contrato_id) == $contrato->id ? 'selected' : '' }}>
            ...
            <div>
                ...
                <select name="autorizacao_fornecimento_id"
                        class="w-full border border-gray-300 rounded-lg px-4 py-3 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors">
                    <option value="">Nenhuma (empenho direto ao processo)</option>
                    @foreach($autorizacoesFornecimento as $af)
                    <option value="{{ $af->id }}" {{ old('autorizacao_fornecimento_id', $empenho->autorizacao_fornecimento_id) == $af->id ? 'selected' : '' }}>
            ...
            <div class="md:col-span-2">
                <label class="flex items-center gap-2">
                    <input type="checkbox" name="concluido" value="1" 
                           {{ old('concluido', $empenho->concluido) ? 'checked' : '' }}
                           class="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded">
                    <span class="text-sm font-semibold text-gray-700">Empenho concluído (entrega realizada)</span>
                </label>
            </div>
            ...
            <div class="md:col-span-2">
                ...
                <textarea name="observacoes" rows="3"
                          ...
                          placeholder="Observações adicionais...">{{ old('observacoes', $empenho->observacoes) }}</textarea>
            </div>
        </div>
