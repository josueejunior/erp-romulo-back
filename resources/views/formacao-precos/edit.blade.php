...
@section('title', 'Editar Formação de Preço - Item ' . $item->numero_item)
...
<div class="mb-8 flex items-center gap-4">
    ...
    <div>
        <h1 class="text-3xl font-bold text-gray-900">Editar Formação de Preço</h1>
        <p class="text-gray-600 mt-1">Item {{ $item->numero_item }} - {{ $orcamento->fornecedor->razao_social }}</p>
    </div>
</div>

<div class="card p-8">
    <form method="POST" action="{{ route('formacao-precos.update', [$processo, $item, $orcamento, $formacaoPreco]) }}" id="formFormacaoPreco">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                ...
                <input type="number" name="custo_produto" id="custo_produto" required step="0.01" min="0"
                       ...
                       value="{{ old('custo_produto', $formacaoPreco->custo_produto) }}" placeholder="0.00">
                @error('custo_produto')
            ...
            <div>
                ...
                <input type="number" name="frete" id="frete" required step="0.01" min="0"
                       class="w-full border border-gray-300 rounded-lg px-4 py-3 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors" 
                       value="{{ old('frete', $formacaoPreco->frete) }}" placeholder="0.00">
                @error('frete')
                    <p class="mt-2 text-sm text-red-600 flex items-center gap-1">
                        <x-heroicon-s-exclamation-circle class="w-4 h-4" />
                        {{ $message }}
            ...
            <div>
                ...
                <input type="number" name="percentual_impostos" id="percentual_impostos" required step="0.01" min="0" max="100"
                       class="w-full border border-gray-300 rounded-lg px-4 py-3 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors" 
                       value="{{ old('percentual_impostos', $formacaoPreco->percentual_impostos) }}" placeholder="0.00">
            ...
            <div>
                ...
                <input type="number" name="percentual_margem" id="percentual_margem" required step="0.01" min="0" max="100"
                       class="w-full border border-gray-300 rounded-lg px-4 py-3 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors" 
                       value="{{ old('percentual_margem', $formacaoPreco->percentual_margem) }}" placeholder="0.00">
            ...
            <div>
                ...
                <input type="number" name="preco_minimo" id="preco_minimo" required step="0.01" min="0"
                       class="w-full border border-gray-300 rounded-lg px-4 py-3 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors" 
                       value="{{ old('preco_minimo', $formacaoPreco->preco_minimo) }}" placeholder="0.00">
            ...
            <div>
                ...
                <input type="number" name="preco_recomendado" id="preco_recomendado" step="0.01" min="0"
                       class="w-full border border-gray-300 rounded-lg px-4 py-3 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors" 
                       value="{{ old('preco_recomendado', $formacaoPreco->preco_recomendado) }}" placeholder="0.00">
            ...
            <div class="md:col-span-2">
                ...
                <textarea name="observacoes" rows="3"
                          ...
                          placeholder="Observações adicionais...">{{ old('observacoes', $formacaoPreco->observacoes) }}</textarea>
            </div>
        </div>
