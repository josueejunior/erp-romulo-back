...
@section('title', 'Novo Orçamento - Item ' . $item->numero_item)
...
<div class="mb-8 flex items-center gap-4">
    ...
    <div>
        <h1 class="text-3xl font-bold text-gray-900">{{ isset($orcamento) ? 'Editar Orçamento' : 'Novo Orçamento' }}</h1>
        <p class="text-gray-600 mt-1">Item {{ $item->numero_item }} - {{ $processo->numero_modalidade }}</p>
    </div>
</div>

<div class="card p-8">
    <form method="POST" action="{{ isset($orcamento) ? route('orcamentos.update', [$processo, $item, $orcamento]) : route('orcamentos.store', [$processo, $item]) }}">
        @csrf
        @if(isset($orcamento))
        @method('PUT')
        @endif
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div class="md:col-span-2">
                ...
                <select name="fornecedor_id" required
                    ...
                    <option value="{{ $fornecedor->id }}" {{ old('fornecedor_id', isset($orcamento) ? $orcamento->fornecedor_id : '') == $fornecedor->id ? 'selected' : '' }}>
            ...
            <div>
                ...
                <input type="number" name="custo_produto" required step="0.01" min="0"
                       class="w-full border border-gray-300 rounded-lg px-4 py-3 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors" 
                       value="{{ old('custo_produto', isset($orcamento) ? $orcamento->custo_produto : '') }}" placeholder="0.00">
                @error('custo_produto')
                    <p class="mt-2 text-sm text-red-600 flex items-center gap-1">
                        <x-heroicon-s-exclamation-circle class="w-4 h-4" />
                        {{ $message }}
            ...
            <div>
                ...
                <input type="text" name="marca_modelo" maxlength="255"
                       class="w-full border border-gray-300 rounded-lg px-4 py-3 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors" 
                       value="{{ old('marca_modelo', isset($orcamento) ? $orcamento->marca_modelo : '') }}" placeholder="Marca e modelo oferecido">
            </div>
            ...
            <div>
                ...
                <input type="number" name="frete" step="0.01" min="0"
                       class="w-full border border-gray-300 rounded-lg px-4 py-3 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors" 
                       value="{{ old('frete', isset($orcamento) ? $orcamento->frete : 0) }}" placeholder="0.00">
            </div>
            ...
            <div>
                ...
                <select name="transportadora_id"
                    ...
                    <option value="{{ $transportadora->id }}" {{ old('transportadora_id', isset($orcamento) ? $orcamento->transportadora_id : '') == $transportadora->id ? 'selected' : '' }}>
            ...
            <div class="md:col-span-2">
                <label class="flex items-center gap-2">
                    <input type="checkbox" name="frete_incluido" value="1" 
                           {{ old('frete_incluido', isset($orcamento) ? $orcamento->frete_incluido : false) ? 'checked' : '' }}
            ...
            <div class="md:col-span-2">
                ...
                <textarea name="ajustes_especificacao" rows="3"
                          ...
                          placeholder="Descreva ajustes ou variações na especificação técnica...">{{ old('ajustes_especificacao', isset($orcamento) ? $orcamento->ajustes_especificacao : '') }}</textarea>
            </div>

            <div class="md:col-span-2">
                <label class="flex items-center gap-2">
                    <input type="checkbox" name="fornecedor_escolhido" value="1" 
                           {{ old('fornecedor_escolhido', isset($orcamento) ? $orcamento->fornecedor_escolhido : false) ? 'checked' : '' }}
                           class="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded">
            ...
            <div class="md:col-span-2">
                ...
                <textarea name="observacoes" rows="3"
                          class="w-full border border-gray-300 rounded-lg px-4 py-3 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors resize-none"
                          placeholder="Observações adicionais...">{{ old('observacoes', isset($orcamento) ? $orcamento->observacoes : '') }}</textarea>
            </div>
        </div>
